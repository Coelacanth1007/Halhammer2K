		...
		for (int i = 0; i < 5; i++)
		{
			...
			if (Light.Flags[i].y == 1)
			{
				if (Light.Flags[i].x == 1)
				{
                    ...
                    float NL = saturate(0.5f + 0.5f * dot(N, L));
                    float3 Diffuse = Material.Diffuse.xyz * NL;

                    float3 R = reflect(-L, N);
                    float3 V = normalize(Camera.xyz - inWorldPos.xyz);
                    float RV = saturate(dot(R, V));
                    float3 Specular = (Material.Specular.xyz + Light.Diffuse[i].xyz) * pow(RV, 6.0f);

                    tempColor = float4(color.xyz * (Ambient + Diffuse + Specular), 1.0f);
