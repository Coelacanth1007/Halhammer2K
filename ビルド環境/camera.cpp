...
#define	POS_Z_CAM			(120.0f)		// カメラの初期位置(Z座標)
...
{
	if (GetKeyboardPress(DIK_Z) || IsButtonPressed(0, STICK_R_RIGHT))
	{// 視点旋回「左」
	...
	if (GetKeyboardPress(DIK_C) || IsButtonPressed(0, STICK_R_LEFT))
	{// 視点旋回「右」
		g_Camera.rot.y -= VALUE_ROTATE_CAMERA;
		if (g_Camera.rot.y < -XM_PI)
		{
...
{
	// カメラの注視点
	g_Camera.at.x = pos.x + sinf(g_Camera.rot.y + 0.35f) * 150.0f;
	g_Camera.at.y = pos.y;
	g_Camera.at.z = pos.z + cosf(g_Camera.rot.y + 0.35f) * 150.0f;



	// カメラの視点をカメラのY軸回転に対応させている
	g_Camera.pos.x = pos.x - sinf(g_Camera.rot.y - 0.35f) * 75.0f;
	g_Camera.pos.z = pos.z - cosf(g_Camera.rot.y - 0.35f) * 75.0f;
	g_Camera.pos.y = pos.y + 15.0f;
