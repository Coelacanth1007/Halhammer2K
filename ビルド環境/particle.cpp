		...
		for(int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
		{
			...
			{// 使用中
				...
				else
				{
					...
					{
						float red	= g_aParticle[nCntParticle].baseColor.x - (float)(80 - g_aParticle[nCntParticle].nLife) / 80 * g_aParticle[nCntParticle].baseColor.x;
						float green = g_aParticle[nCntParticle].baseColor.y - (float)(80 - g_aParticle[nCntParticle].nLife) / 80 * g_aParticle[nCntParticle].baseColor.y;
						float blue	= g_aParticle[nCntParticle].baseColor.z - (float)(80 - g_aParticle[nCntParticle].nLife) / 80 * g_aParticle[nCntParticle].baseColor.z;
						g_aParticle[nCntParticle].material.Diffuse.x = red;
						g_aParticle[nCntParticle].material.Diffuse.y = green;
						g_aParticle[nCntParticle].material.Diffuse.z = blue;
					}
...
int SetParticle(XMFLOAT3 pos, XMFLOAT3 move, XMFLOAT4 col, float fSizeX, float fSizeY, int nLife)
{
	...
	for(int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		if(!g_aParticle[nCntParticle].use)
		{
			...
			g_aParticle[nCntParticle].scale = { 0.2f, 0.2f, 0.2f };
			g_aParticle[nCntParticle].move = move;
			g_aParticle[nCntParticle].baseColor = col;
...
int SetParticle(XMFLOAT3 pos, XMFLOAT3 scl, XMFLOAT3 move, XMFLOAT4 col, int nLife)
{
	...
	for (int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		if (!g_aParticle[nCntParticle].use)
		{
			...
			g_aParticle[nCntParticle].baseColor = col;
			g_aParticle[nCntParticle].material.Diffuse = col;
			g_aParticle[nCntParticle].fSizeX = 0.0f;
			g_aParticle[nCntParticle].fSizeY = 0.0f;
			g_aParticle[nCntParticle].nLife = nLife;
			g_aParticle[nCntParticle].use = TRUE;
...
int SetOrcParticle(XMFLOAT3 pos, XMFLOAT3 move, XMFLOAT4 col, float fSizeX, float fSizeY, int nLife)
{
	...
	for(int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		if(!g_aParticle[nCntParticle].use)
		{
			g_aParticle[nCntParticle].pos = pos;
			g_aParticle[nCntParticle].rot   = { 0.0f, 0.0f, 0.0f };
			g_aParticle[nCntParticle].scale = { 2.0f, 3.0f, 2.0f };
			g_aParticle[nCntParticle].move = move;
			g_aParticle[nCntParticle].material.Diffuse = col;
			g_aParticle[nCntParticle].fSizeX = fSizeX;
			g_aParticle[nCntParticle].fSizeY = fSizeY;
