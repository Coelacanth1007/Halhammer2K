...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		//* y-axis (forward)
		if ( dijs.lY < 0 )					padState[i] |= STICK_L_UP;
		//* y-axis (backward)
		if (dijs.lY > 0)					padState[i] |= STICK_L_DOWN;
		//* x-axis (left)
		if (dijs.lX < 0)					padState[i] |= STICK_L_LEFT;
		//* x-axis (right)
		if (dijs.lX > 0)					padState[i] |= STICK_L_RIGHT;
		
		//* y-axis (forward)
		if (dijs.lRz < 0)					padState[i] |= STICK_R_UP;
		//* y-axis (backward)									 
		if (dijs.lRz > 0)					padState[i] |= STICK_R_DOWN;
		//* x-axis (left)										 
		if (dijs.lZ < 0)					padState[i] |= STICK_R_LEFT;
		//* x-axis (right)										 
		if (dijs.lZ > 0)					padState[i] |= STICK_R_RIGHT;
